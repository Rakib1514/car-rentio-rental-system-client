...
const Banner = () => {
  ...
  useEffect(()=>{
    axios.get('/random-image')
      .then(res=> setBannerImg(res.data[0].image))
  },[])
    ...
    >
      ...
      <div className="hero-content text-neutral-content text-center">
        <div className="max-w-2xl bg-black backdrop-blur-md bg-opacity-25 p-3 rounded-md">
          ...
          <Link to={'/available-cars'} className="btn btn-outline bg-primary font-bold text-white font-openSans">Find Your Ride</Link>
        </div>
      </div>
    </div>
  );
